...
import com.vinapp.tradernettestapp.base.network.outgoing.EventSubscriptionParametersSerializer
...
import com.vinapp.tradernettestapp.base.network.income.WebSocketEvent
import com.vinapp.tradernettestapp.base.network.income.WebSocketEventData
import com.vinapp.tradernettestapp.base.network.income.WebSocketEventSerializer
...
class WebSocketControllerImpl(
    ...
    override fun <T : WebSocketEventData> subscribeOnEvents(params: EventSubscriptionParameters): Flow<T> {
        ...
        webSocketScope.launch {
            sessionStateFlow.filterNotNull().collect { session ->
                val message = format.encodeToString(EventSubscriptionParametersSerializer, params)
        ...
        return eventsFlow.filterNotNull()
            .filter { event ->
                val incomingEventType = when (params) {
                    is QuoteChangeEventSubscriptionParams -> EventType.QUOTE_UPDATE
                    else -> null
                }
